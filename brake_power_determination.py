...
def get_required_signals(data):
    ...
    return (dt, time, speed_meters_per_second, acceleration, power_at_wheels, mass, grade, 
            transmission_output_speed, overall_ratio, engine_speed, accelerator_pedal)
...
def prepare_data_for_ml_model_training(data):
    ...
    (_,
     ...
     overall_ratio,
     engine_speed,
     _)  = get_required_signals(data)
    
    X       = np.c_[speed_meters_per_second, acceleration, power_at_wheels, mass, grade]
    slip    = transmission_output_speed*overall_ratio/(engine_speed + np.finfo(float).eps) # safe divide
    
    positive_power_index = np.where((acceleration >= 0) & (speed_meters_per_second > 0) & (slip <= 1.0))[0] 
    X_positive_power = X[positive_power_index, :]
    x_model_input, y_to_fit, inertia_power, gradient_power  = transform_data(X_positive_power, model='lin_reg')
...
def train_road_load_model(x_model_input, y_to_fit):
    ...
    x_train, x_test, y_train, y_test = train_test_split(x_model_input, y_to_fit, test_size=0.30)
    ...
    return model, coefficients, scaled_coefficients, train_rmse, train_gof, test_rmse, test_gof
