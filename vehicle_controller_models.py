...
class simple_vehicle_controller:
    ...
    def _wheel_power_limit_calculation(self, battery, electric_machine):
        _battery_discharging_mechanical_limit = (
            -battery.discharging_power_limit * electric_machine.motoring_efficiency
        )
        _machine_motoring_mechanical_limit = min(
            electric_machine.motoring_mechanical_power_limit,
            (
                electric_machine.motoring_electrical_power_limit
                * electric_machine.motoring_efficiency
            ),
        )
        self._propulsion_wheel_power_limit = min(
            _battery_discharging_mechanical_limit * self.driveline_efficiency,
            _machine_motoring_mechanical_limit * self.driveline_efficiency,
        )

        _battery_charging_mechanical_limit = (
            -battery.charging_power_limit / electric_machine.generating_efficiency
        )
        _machine_generating_mechanical_limit = max(
            electric_machine.generating_mechanical_power_limit,
            electric_machine.generating_electrical_power_limit
            / electric_machine.generating_efficiency,
        )
        self._generation_wheel_power_limit = max(
            _battery_charging_mechanical_limit / self.driveline_efficiency,
    ...
    def _regen_power_demand_electric_machine(self, speed, battery):
        ...
        if _battery_soc_limit or _vehicle_speed_limit:
            ...
            if self._powertrain_power_demand <= 0:
                ...
                _regen_braking_power_raw = (
                    self._brake_power_demand
                    * self.regen_electric_to_friction_braking_ratio
                )
                _regen_coasting_power_limited = (
                    max(self._generation_wheel_power_limit, _regen_coasting_power_raw)
                    * self.driveline_efficiency
                )
                _residual_limit_after_coasting = (
                    self._generation_wheel_power_limit
                    - _regen_coasting_power_limited / self.driveline_efficiency
                )
                _regen_braking_power_limited = (
                    max(_residual_limit_after_coasting, _regen_braking_power_raw)
                    * self.driveline_efficiency
                )

            elif (
                self._brake_power_demand + self._powertrain_power_demand
            ) < 0:  # Driver wants to decelerate but filtered powertrain demand may not be 0
        ...
        self._regen_power_demand = (
            _regen_coasting_power_limited + _regen_braking_power_limited
        )
    ...
    def _propulsion_power_demand_electric_machine(self, battery):
        ...
        if _battery_soc_limit:
            ...
            if (
                self._powertrain_power_demand + self._brake_power_demand
            ) >= 0:  # driver wants to accelerate overall at constrained speed
                self._propulsion_power_demand = (
                    min(
                        self._propulsion_wheel_power_limit,
                        (self._powertrain_power_demand + self._brake_power_demand),
                    )
                    / self.driveline_efficiency
                )
    ...
    def calculate_electric_machine_power_demand(
        self, powertrain_torque, brake_torque, speed, battery, electric_machine
    ):
