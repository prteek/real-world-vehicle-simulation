...
overall_ratio,
engine_speed,
accelerator_pedal)  = get_required_signals(data)
...
train_gof, 
test_rmse, 
test_gof) = train_road_load_model(x_model_input, y_to_fit)
...
battery  = constant_parameter_battery(initial_soc=1) # 100% charged at the start of the day
...
logged_propulsion_energy      = np.trapz(power_at_wheels*((power_at_wheels+brake_power)>0), time)
...
logged_number_of_trips          = (np.sum((np.diff(data['Ign_Voltage']<2)>0))-1)/2 + 1 # pair wise on-off and 2 off for 1st trip
...
plug_in_charge_power               = battery.capacity*(battery.maximum_battery_soc - battery.minimum_battery_soc)/battery.plug_full_charge_time
...
for i in range(len(timestamp)):
    ...
    if (timestamp[i] - time_previous) > opportunistic_charging_threshold:
        ev_opportunistic_potential_charge_time   = ev_opportunistic_potential_charge_time + (timestamp[i] - time_previous)
        ev_opportunistic_potential_charge_events += 1
        
        # Opportunistic charging
        battery_capacity_buffer               = battery.capacity*(battery.maximum_battery_soc - battery.soc)
        battery_charging_time_valid           = min(battery_capacity_buffer/plug_in_charge_power, (timestamp[i] - time_previous))
        ev_opportunistic_charge_time          = ev_opportunistic_charge_time + battery_charging_time_valid
        ev_opportunistic_charge_energy        = ev_opportunistic_charge_energy + plug_in_charge_power*battery_charging_time_valid
    ...
    # normal operation
    vsc.calculate_electric_machine_power_demand(wheel_torque[i], brake_torque[i], wheel_speed[i], battery, electric_machine)
    electric_machine.calculate_battery_current_demand(vsc.electric_machine_power_demand, battery)
    battery.update_battery_state(electric_machine.battery_current_demand, electric_machine, dt)
...
ev_battery_propulsion_energy               = np.trapz(battery_current*battery_voltage*(vsc_mode==1), time)
ev_battery_regen_energy                    = np.trapz(battery_current*battery_voltage*(vsc_mode==-1), time)
ev_battery_max_propulsion_power            = np.min(battery_current*battery_voltage*(vsc_mode==1))
ev_battery_max_regen_power                 = np.max(battery_current*battery_voltage*(vsc_mode==-1))
ev_end_of_day_soc                          = max(battery.soc, 0)
ev_min_soc                                 = np.min(battery_soc)
ev_average_soc                             = np.mean(battery_soc[battery_soc>0])
ev_fuel_saved                              = np.trapz(data['Fuel_Rate']*(battery_soc>0),time)/3600/1000 # [m^3/sec]

file_results            = pd.DataFrame([[coefficients, 
                scaled_coefficients, 
                train_rmse, 
                train_gof, 
                test_rmse,
