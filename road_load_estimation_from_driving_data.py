...
time                    = data['time']
dt                      = time[1] - time[0]
engine_torque           = np.array(data['Net_Teng'], dtype='float') # Use net torque for positive and negative values
...
def fo_lp_filter(data, tc=1):
    ...
    for i in range(samples-1):
        ...
        actual_value = previous_actual + (target_value - previous_actual)*(1 - np.exp(-1/tc))
...
power_at_wheels_raw = engine_torque*overall_ratio*transmission_output_speed/30*np.pi
...
speed_meters_per_second_filtered = np.convolve(speed, filter_mov_mean, mode='same')*(speed!=0)/3.6
acceleration            = np.append(np.diff(speed_meters_per_second_filtered)/dt,0)

speed_meters_per_second = speed/3.6     # if not converted to float already, use /3.6 since speed is uint8

# Filter power at wheels since it is calculated from engine torque which has much higher dynamics
pos_power_at_wheels        = fo_lp_filter(power_at_wheels_raw, low_pass_tc)*(power_at_wheels_raw>0)
neg_power_at_wheels        = fo_lp_filter(power_at_wheels_raw, low_pass_tc)*(power_at_wheels_raw<0)
...
X       = np.c_[speed_meters_per_second, acceleration, power_at_wheels, mass, grade, overall_ratio]
...
x_train, x_test, y_train, y_test = train_test_split(x_model_input, y_to_fit, test_size=0.20)

lin_reg = LinearRegression(fit_intercept=False) # set this to false as we don't want any constant term to be fit
scores  = cross_val_score(lin_reg, x_train, y_train, scoring='neg_mean_squared_error', cv=30)
...
print('RMSE train lr:', np.round(np.sqrt(mean_squared_error(train_predict, y_train)),0), ' \t GOF:', np.round(r2_score(train_predict, y_train),2))

test_predict  = lin_reg.predict(x_test)
print('RMSE test lr:', np.round(np.sqrt(mean_squared_error(test_predict, y_test)),0), ' \t GOF:', np.round(r2_score(test_predict, y_test),2))
...
x_train, x_test, y_train, y_test = train_test_split(x_model_input, y_to_fit, test_size=0.20, random_state=42)
...
grid_search = GridSearchCV(rfr, param_grid, cv=8,
...
print('RMSE train rf:', np.round(np.sqrt(mean_squared_error(train_predict, y_train)),0), ' \t GOF:', np.round(r2_score(train_predict, y_train),2))

x_test       = pca.transform(x_test)
test_predict = rfr.predict(x_test)
print('RMSE test rf:', np.round(np.sqrt(mean_squared_error(test_predict, y_test)),0), ' \t GOF:', np.round(r2_score(test_predict, y_test),2))
...
tic = TT.time()
scores  = cross_val_score(rfr, x_train, y_train, scoring='neg_mean_squared_error', cv=32, n_jobs=4)
...
x_values, power_to_fit, inertia_power, gradient_power = transform_data(X, model='lin_reg')

brake_power_raw = power_to_fit - lin_reg.predict(x_values)
brake_power = brake_power_raw*((power_at_wheels-brake_power_raw)<=0)*(accelerator_pedal==0)*(brake_power_raw>=0)
...
x_values_rfr, power_to_fit_rfr, inertia_power, gradient_power = transform_data(X, model='rfr')
...
for i in range(len(random_data_points)):
    xx = np.hstack((x_values_rfr[:, 0].reshape(-1,1), 
...
fig = plt.figure()
plt.plot(speed_meters_per_second, rfr.predict(x_values_rfr),'.', alpha=0.1, label='random forest model')
plt.plot(speed_meters_per_second, lin_reg.predict(x_values),'.', alpha=0.1, label='linear regression')
plt.legend()
plt.grid()
plt.show()
